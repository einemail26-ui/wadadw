...
TWITTER_CONFIGS = {
    "normiesART": {
        "api_key":             _env("TW_API_KEY",             "vU9YN0PH66FlZ7KaH67HRVJ8g"),
        "api_secret":          _env("TW_API_SECRET",          "XmZk8Kt0KccumPfSzfOJKWU1ht2VvHdc3CHZFmVVq4WIfV2M7e"),
        "access_token":        _env("TW_ACCESS_TOKEN",        "2039233498058223617-rLy3FV7pKSE4T5FXdEu5I5FJMkidtc"),
        "access_token_secret": _env("TW_ACCESS_TOKEN_SECRET", "Rbfu43ufW1Lwz7CuHtTvX7J24CDw2R37EZ8i6TfZGWUs0"),
        "bearer_token":        _env("TW_BEARER_TOKEN",        "AAAAAAAAAAAAAAAAAAAAAHPE8gEAAAAAoe16YJk%2FOlLYaWjuFGGsEoDVKzc%3DUsMN7pXsSPneUOx8HI9t6dFsIZupoxMHjhpKb0gGj2Q0VebE7k"),
    },
}
...
KILLSWITCH_ACCOUNTS = [
    "normiesartbot",   # bot account itself
    # add more trusted accounts here
]
...
def flush_queue():
    ...
    _next_tweet_at = time.time() + random.randint(30, 90)
...
def tweet_burn(receiver_id, count, owner, ap=0):
    ...
    ap_str = f"{ap} AP" if ap else "AP"
    if count >= 20:
        templates = [
            f"Legendary move: {ens} burned {count} {plural} to score {ap_str} for Normie #{receiver_id}. On-chain history being written right now. 📜\n{link}\n\n@normiesART #normies #NormiesCanvas",
            f"Massive burn! {ens} just sacrificed {count} {plural} for {ap_str} on Normie #{receiver_id}. That's how you build a masterpiece. 🎨\n{link}\n\n@normiesART #normies #NormiesCanvas",
        ]
    else:
        templates = [
            f"Burn alert! {ens} torched {count} {plural} and earned {ap_str} to upgrade Normie #{receiver_id}. Pixel evolution in progress. 🔥\n{link}\n\n@normiesART #normies #NormiesCanvas",
            f"{ens} just burned {count} {plural} to get {ap_str} for Normie #{receiver_id}. Sacrifice for the canvas! 🔥\n{link}\n\n@normiesART #normies #NormiesCanvas",
        ]
    return random.choice(templates)
...
def tweet_canvas(token_id, ap, changes):
    ...
    templates = [
        ...
        f"🎨 Artist at work! Normie #{token_id} just got {changes} pixels updated with {ap} AP.\n{link}\n\n@normiesART #normies #NormiesCanvas",
    ]
    return random.choice(templates)
...
def opensea_link(token_id):
    return f"https://opensea.io/assets/ethereum/0x9Eb6E2025B64f340691e424b7fe7022fFDE12438/{token_id}"
...
def top_spenders(db, slug, days=1, n=5):
    ...
    for i in range(days):
        for b, s in db["collections"][slug]["history"].get(
...
def post_rankings(col, db, now):
    ...
    if td:
        lines = "\n".join(
            f"{em[i]} {b[:6]}...{b[-4:]} — {e:.3f} ETH ({fmt_usd(e)})"
            for i, (b, e) in enumerate(td)
        )
        post_all(col,
            ...
            post_all(col,
                ...
                {"title": "⭐ Collector of the Week!", "color": 0xFFD700,
                 "fields": [
                     ...
                     {"name": "Spent",  "value": f"{eth_spent:.3f} ETH ({fmt_usd(eth_spent)})", "inline": True}
                 ],
                 "timestamp": datetime.datetime.utcnow().isoformat()}
            )
    ...
    while True:
        ...
        if not db.get("bot_paused", False):
            ...
            for col in COLLECTIONS:
                ...
                except Exception as e: log.error(f"[Milestone ERR] {e}")
            ...
            if now.hour == DAILY_HOUR and now.minute == 0 and last_rank != now.date():
                ...
                for col in COLLECTIONS:
                    ...
                    except Exception as e: log.error(f"[Ranking ERR] {e}")
                last_rank = now.date()

        save_db(db)

        # Flush one tweet from queue per loop
        flush_queue()
