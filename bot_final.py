...
def process_sales(col, db):
    ...
    # Limit to MAX_SALES_PER_LOOP — skip older ones to avoid flooding
    if len(new_sales) > MAX_SALES_PER_LOOP:
        log.info(f"[Sales] {len(new_sales)} new sales, capping at {MAX_SALES_PER_LOOP} to avoid flood.")
        new_sales = new_sales[:MAX_SALES_PER_LOOP]
