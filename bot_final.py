...
MIN_SECONDS_BETWEEN  = 45   # min pause between any two tweets

# Queue: max pending tweets
MAX_QUEUE_SIZE = 20
TWEET_TTL      = 420  # 7 minutes — drop stale tweets from queue
MAX_SALES_PER_LOOP = 5   # max sales to queue per check cycle
...
def queue_tweet(acc, text, priority=False, media_id=None):
    ...
    if len(_tweet_queue) >= MAX_QUEUE_SIZE:
        ...
        _tweet_queue.pop(0)
    entry = {"acc": acc, "text": text, "ts": time.time(), "media_id": media_id}
    if priority:
...
def flush_queue():
    ...
    # Drop stale tweets older than TWEET_TTL
    now = time.time()
    stale = [e for e in _tweet_queue if now - e["ts"] > TWEET_TTL]
    for e in stale:
        _tweet_queue.remove(e)
        log.info(f"[Queue] Dropped stale tweet (>{TWEET_TTL}s old): {e['text'][:60]}...")
    if not _tweet_queue:
        return
    if not _rate_limiter.can_tweet():
        return
    if now < _next_tweet_at:
    ...
    _post_tw_now(entry["acc"], entry["text"], media_id=entry.get("media_id"))
...
def tweet_single_sale(token_id, price_eth, price_usd, floor=0):
    if floor > 0:
        multiple = price_eth / floor
        multiple_str = f" — {multiple:.1f}x floor"
    else:
        multiple_str = ""
    return f"Normie #{token_id} sold for {price_eth:.4f} ETH ({price_usd}){multiple_str}\n\n@normiesART"

def tweet_sweep(count, total_eth, total_usd, buyer):
    ens = get_ens_name(buyer)
    return f"{count} Normies swept for {total_eth:.3f} ETH ({total_usd}) by {ens}\n\n@normiesART"

def tweet_grail(token_id, ntype, price_eth, price_usd):
    return f"Normie #{token_id} [{ntype}] sold for {price_eth:.4f} ETH ({price_usd}) — grail\n\n@normiesART"
...
def tweet_burn(receiver_id, count, owner, ap=0):
    ...
    return f"{count} {plural} burned for Normie #{receiver_id} by {ens}\n\n@normiesART"

def tweet_canvas(token_id, ap, changes):
    return f"Normie #{token_id} edited — {changes} pixels changed\n\n@normiesART"

def tweet_floor(direction, pct, new_floor, new_usd):
    sign = "+" if direction == "up" else "-"
    return f"Floor {new_floor:.4f} ETH ({new_usd}) {sign}{pct:.1f}%\n\n@normiesART"
        ...
        for col in COLLECTIONS:
            ...
            if s not in db["collections"]:
                db["collections"][s] = {
                    ...
                    "last_canvas_tx": {},
                    "last_floor":          None,
                    "last_floor_alert_ts": 0,
                    "total_volume":   0.0,
                    "history":        {},
                    "buyer_window":   {},
                    "posted_txs":          [],   # dedupe: last 200 posted tx hashes
                    "last_killswitch_mention_id": None,  # dedupe killswitch mentions
                }
...
def check_killswitch(acc_name):
    ...
    try:
        ...
        db      = load_db()
        last_id = db.get("last_killswitch_mention_id")
        changed = False

        # Use since_id to only get new mentions since last check
        kwargs = {
            "id":           bot_id,
            "max_results":  5,
        ...
        for mention in mentions.data:
            ...
            if newest_id is None or int(mention_id) > int(newest_id):
        ...
        # Always save newest mention ID so we never process it again
        if newest_id:
            db["last_killswitch_mention_id"] = newest_id
            changed = True
...
_posted_texts = []  # recent tweet texts for duplicate check
MAX_POSTED_TEXTS = 50

def _post_tw_now(acc, text, media_id=None):
    """Actually post to Twitter — called only from flush_queue."""
    global _posted_texts
    # Duplicate text check — avoid 403 duplicate content error
    if text in _posted_texts:
        log.warning(f"[TW] Duplicate tweet skipped: {text[:60]}...")
        return
    try:
        kwargs = {"text": text}
        if media_id:
            kwargs["media_ids"] = [media_id]
        get_tw(acc).create_tweet(**kwargs)
        _rate_limiter.record_tweet()
        _posted_texts.append(text)
        _posted_texts = _posted_texts[-MAX_POSTED_TEXTS:]  # keep last 50
...
def post_all(col, tw_text, dc_embed, priority=False, media_id=None):
    """Queue tweet + immediately send to Discord."""
    db = load_db()
    if db.get("bot_paused"):
        log.info("[PAUSED] Bot is paused, skipping post.")
        return
    queue_tweet(col["twitter_account"], tw_text, priority=priority, media_id=media_id)
...
def upload_normie_image(acc_name, token_id):
    """Download Normie image and upload to Twitter. Returns media_id or None."""
    try:
        img_url = get_normie_img(token_id)
        r = requests.get(img_url, timeout=10)
        if r.status_code != 200:
            log.warning(f"[Media] Failed to download image for #{token_id}")
            return None

        # Upload via v1.1 API (required for media upload)
        c = TWITTER_CONFIGS[acc_name]
        ...
        media = api.media_upload(filename=f"normie_{token_id}.png", file=img_bytes)
        log.info(f"[Media] Uploaded image for #{token_id} — media_id: {media.media_id}")
        return media.media_id
    except Exception as e:
        log.warning(f"[Media] Upload failed for #{token_id}: {e}")
        return None
...
def process_sales(col, db):
    ...
    sales = get_recent_sales(slug, limit=50)
    if not sales: return

    # Collect ALL new sales since last tx — no cap here for sweep detection
    all_new_sales = []
    for s in sales:
        if s["tx"] == cd["last_sale_tx"]: break
        all_new_sales.append(s)
    if not all_new_sales: return
    ...
    # ── STEP 1: Build buyer window from ALL new sales for sweep detection ──
    for s in reversed(all_new_sales):
        buyer = s["buyer"]
        if buyer not in bw: bw[buyer] = []
        bw[buyer].append(now_ts)

    # ── STEP 2: Detect sweeps across ALL new sales ──
    sweep_posted = set()
    for buyer, times in bw.items():
        buyer_sales     = [s for s in all_new_sales if s["buyer"] == buyer]
        buyer_total_eth = sum(s["price"] for s in buyer_sales)
        cnt             = len(times)

        if cnt >= col["sweep_count"] and buyer_total_eth >= col["sweep_min_eth"]:
            ...
            bw[buyer] = []  # reset window after posting
            log.info(f"[Sweep] {cnt} Normies by {short} for {buyer_total_eth:.3f} ETH")

    # ── STEP 3: Post individual sales — cap applies here only ──
    sales_to_post = [s for s in all_new_sales if s["buyer"] not in sweep_posted]
    if len(sales_to_post) > MAX_SALES_PER_LOOP:
        log.info(f"[Sales] {len(sales_to_post)} individual sales, capping at {MAX_SALES_PER_LOOP}.")
        sales_to_post = sales_to_post[:MAX_SALES_PER_LOOP]

    posted_txs = cd.get("posted_txs", [])
    for s in reversed(sales_to_post):
        tx       = s["tx"]
        buyer    = s["buyer"]
        price    = s["price"]
        token_id = s["tokenId"]
        ...
        if price >= col["sale_min_eth"]:
            ...
            if is_grail:
                ...
                color    = 0x9B59B6
                media_id = upload_normie_image(col["twitter_account"], token_id)
            else:
                floor    = get_cached_floor(col["slug"])
                tw       = tweet_single_sale(token_id, price, usd, floor)
                color    = 0x00CC66
                media_id = None  # no image for regular sales
            ...
            post_all(col, tw, dc, priority=is_grail, media_id=media_id)

        cd["total_volume"] = cd.get("total_volume", 0.0) + price
        if tx not in posted_txs:
            posted_txs.append(tx)

    # ── STEP 4: Always advance last_sale_tx to newest ──
    cd["last_sale_tx"] = all_new_sales[0]["tx"]  # newest is first
    cd["posted_txs"]   = posted_txs[-200:]
    cd["buyer_window"] = bw
    log.info(f"[{col['name']}] {len(all_new_sales)} new sales processed.")

# ================================================================
# FEATURE 2: BURNS
# ================================================================

BURN_BATCH_WINDOW = 600  # 10 min — group burns from same owner
...
def check_burns(col, db):
    ...
    if not new_burns: return

    # Batch burns by owner — group multiple burns from same owner
    batches = {}  # owner -> {receiver, total_count, ap, latest_commit}
    for burn in reversed(new_burns):
        receiver   = burn.get("receiverTokenId", "?")
        count      = burn.get("tokenCount", 0)
        owner      = burn.get("owner", "")
        commit_id  = str(burn.get("commitId"))

        if owner not in batches:
            batches[owner] = {"receiver": receiver, "count": 0, "commit_id": commit_id}
        batches[owner]["count"]     += count
        batches[owner]["commit_id"]  = commit_id  # keep latest
    ...
    # Save last burn ID
    if new_burns:
        cd["last_burn_id"] = str(new_burns[0].get("commitId"))
...
def check_floor(col, db):
    ...
    last_alert_ts    = cd.get("last_floor_alert_ts", 0)
    floor_cooldown   = 1800  # 30 minutes between floor alerts

    if last and last > 0:
        pct = ((floor - last) / last) * 100
        now = time.time()
        cooldown_ok = (now - last_alert_ts) >= floor_cooldown
        if abs(pct) >= col.get("floor_alert_pct", 20) and cooldown_ok:
            ...
            post_all(col, tw, dc)
            cd["last_floor_alert_ts"] = now
        elif abs(pct) >= col.get("floor_alert_pct", 20) and not cooldown_ok:
            remaining = int((floor_cooldown - (now - last_alert_ts)) / 60)
            log.info(f"[Floor] Alert suppressed — cooldown {remaining}min remaining")
    cd["last_floor"] = floor
...
if __name__ == "__main__":
    try:
        ...
        log.info("[Shutdown] Bot stopped cleanly.")
    except Exception as e:
        log.critical(f"[Shutdown] Unexpected crash: {e}")
